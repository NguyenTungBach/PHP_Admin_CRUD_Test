...
class OrderController extends Controller
{
    public function getDetail($id)
    {
        $order = Order::find($id);
        if ($order ==null){
            return view('client.demo.404order',['msg' => 'Order not found']);
        }
        return view('client.demo.detailOrder',['order'=> $order]);
    }
...
// information
        ...
        foreach ($order->orderDetails as $orderDetail){
            $item = new Item();
            $item->setName($orderDetail->product->description)
                ->setCurrency('USD')
...
// is fulfilling it.
        ...
        $transaction->setAmount($amount)
            ...
            ->setInvoiceNumber($order->id); // set id Order
...
//        $execution->addTransaction($transaction);
    ...
    public function getCheckMail(Request $request){
        ...
        return view('client.demo.mailOrder.orderMail',['order'=> $order]);
    }
}
